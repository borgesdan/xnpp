...
namespace Xna {
	SongContent SongProcessor::ProcessT(AudioContent& input, ContentProcessorContext& context) {
		if (!input.FileType || 
			(input.FileType && input.FileType != AudioFileType::Mp3 
				&& input.FileType != AudioFileType::Wav 
				&& input.FileType != AudioFileType::Ogg
				&& input.FileType != AudioFileType::Wma))
			throw std::runtime_error("Unsupported format: Song processor supports MP3, WMA, OGG and WAV files.");
