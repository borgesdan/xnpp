...
namespace Xna {
	...
	// Garante o alinhamento correto para evitar padding indesejado pelo compilador
#pragma pack(push, 1)
	...
	public:
		...
		constexpr bool IsKeyDown(Keys key) const {
			...
			if (k <= 0xfe) {
				const unsigned int index = k >> 5;     // k / 32
				const unsigned int bitMask = 1u << (k & 0x1f); // k % 32

				// Agora acessamos o array diretamente da union, sem reinterpret_cast!
		...
		KeyState operator[](Keys key) const {
			return static_cast<KeyState>(IsKeyDown(key) ? 1 : 0);
		}

		// A union permite acessar os mesmos 32 bytes de duas formas diferentes
		union {
...
#pragma pack(pop)
	...
	// Validação em tempo de compilação para garantir que o tamanho nunca ultrapasse 256 bits (32 bytes)
	static_assert(sizeof(KeyboardState) == 32, "KeyboardState deve ter exatamente 32 bytes.");

	//Allows retrieval of keystrokes from a keyboard input device. 
	struct Keyboard final {
	public:
		//Returns the current keyboard. 
		XNPP_API static KeyboardState GetState();
